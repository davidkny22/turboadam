...
def _make_test_data(numel=768*768, n_bits=4, block_size=128, device='cuda'):
    ...
    compressed = compress_v_logscale(v.reshape(-1), n_bits=n_bits, block_size=block_size)
...
class TestTritonVUpdate:
    def test_correctness_vs_pytorch(self):
        ...
        pt_idx, pt_scales, pt_v = pytorch_fused_v_update(
            comp["indices"].clone(), comp["scales"].clone(), grad.clone(), beta2,
            comp["n_bits"], comp["block_size"], comp["original_length"],
        )
    ...
    def test_output_shapes(self):
        ...
        new_idx, new_scales, v_flat = triton_fused_v_update(
            comp["indices"], comp["scales"], grad, 0.999,
            comp["n_bits"], comp["block_size"], comp["original_length"],
        )
    ...
    def test_performance(self):
        ...
        for _ in range(5):
            pytorch_fused_v_update(
                comp["indices"].clone(), comp["scales"].clone(), grad, 0.999,
                comp["n_bits"], comp["block_size"], comp["original_length"],
            )
