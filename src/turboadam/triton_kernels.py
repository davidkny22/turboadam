...
@triton.jit
def _multi_tensor_v_update_kernel(
    indices_ptr,
    scales_ptr,
    grads_ptr,
    v_out_ptr,
    step_seed_ptr,
    param_block_offsets_ptr,
    param_elem_offsets_ptr,
    param_numels_ptr,
    n_params: tl.constexpr,
    total_blocks,
    beta2: tl.constexpr,
    one_minus_beta2: tl.constexpr,
    n_buckets: tl.constexpr,
    ...
    for global_block_id in range(pid, total_blocks, n_programs):
        param_id = tl.full((), 0, tl.int64)
        for i in range(0, n_params):
            block_start_i = tl.load(param_block_offsets_ptr + i)
            param_id = tl.where(global_block_id >= block_start_i, i, param_id)

        param_block_start = tl.load(param_block_offsets_ptr + param_id)
        param_elem_start = tl.load(param_elem_offsets_ptr + param_id)
        param_numel = tl.load(param_numels_ptr + param_id)
        local_block_id = global_block_id - param_block_start
        local_offs = local_block_id * BLOCK_SIZE + arange
        elem_offs = param_elem_start + local_offs
        mask = local_offs < param_numel

        old_idx = tl.load(indices_ptr + elem_offs).to(tl.int32)
        log_min = tl.load(scales_ptr + global_block_id * 2).to(tl.float32)
        log_max = tl.load(scales_ptr + global_block_id * 2 + 1).to(tl.float32)
        span = log_max - log_min
        log_v = log_min + (old_idx.to(tl.float32) + 0.5) / n_buckets * span
        v_old = tl.exp(log_v)

        g = tl.load(grads_ptr + elem_offs, mask=mask, other=0.0)
        ...
        new_log_max = tl.max(log_v_new, axis=0)
        new_span = tl.maximum(new_log_max - new_log_min, 1e-10)
        continuous = ((log_v_new - new_log_min) / new_span) * n_buckets

        floor_idx = tl.math.floor(continuous)
        frac = continuous - floor_idx
        step_seed = tl.load(step_seed_ptr).to(tl.uint32)
        rand_seed = step_seed + global_block_id.to(tl.uint32)
        rand_val = _rand_uniform_u32(rand_seed, elem_offs)
        rounded = floor_idx + (rand_val < frac).to(tl.float32)
        new_idx = tl.minimum(tl.maximum(rounded, 0.0), (n_buckets - 1) * 1.0).to(tl.uint8)

        tl.store(indices_ptr + elem_offs, new_idx)
        tl.store(scales_ptr + global_block_id * 2, new_log_min.to(tl.float16))
        tl.store(scales_ptr + global_block_id * 2 + 1, new_log_max.to(tl.float16))
        tl.store(v_out_ptr + elem_offs, v_new, mask=mask)


def triton_multi_tensor_v_update(
    indices: torch.Tensor,
    scales: torch.Tensor,
    grads: torch.Tensor,
    v_out: torch.Tensor,
    step_seed: torch.Tensor,
    param_block_offsets: torch.Tensor,
    param_elem_offsets: torch.Tensor,
    param_numels: torch.Tensor,
    beta2: float,
    n_bits: int,
    block_size: int,
) -> None:
    """Update compressed v for a whole param group in one Triton launch."""
    n_params = param_numels.numel()
    total_blocks = scales.shape[0]
    n_programs = min(total_blocks, 16384)

    _multi_tensor_v_update_kernel[(n_programs,)](
        indices, scales.reshape(-1), grads, v_out, step_seed,
        param_block_offsets, param_elem_offsets, param_numels,
        n_params=n_params,
...
@triton.jit
def _multi_tensor_v_update_weight_kernel(
    indices_ptr,
    scales_ptr,
    grads_ptr,
    param_ptrs_ptr,
    ...
    n_programs = tl.num_programs(0)
    arange = tl.arange(0, BLOCK_SIZE)
    bc1 = tl.load(bc1_ptr).to(tl.float32)
    bc2 = tl.load(bc2_ptr).to(tl.float32)
    ...
    for global_block_id in range(pid, total_blocks, n_programs):
        ...
        param_ptr = tl.load(param_ptrs_ptr + param_id).to(tl.pointer_type(tl.float32))
        m_ptr = tl.load(m_ptrs_ptr + param_id).to(tl.pointer_type(tl.float32))
        m = tl.load(m_ptr + local_offs, mask=mask, other=0.0)
        p = tl.load(param_ptr + local_offs, mask=mask, other=0.0)
        denom = tl.sqrt(v_new / bc2) + eps
        p_new = p - lr * m / (denom * bc1)

        tl.store(indices_ptr + elem_offs, new_idx)
        tl.store(scales_ptr + global_block_id * 2, new_log_min.to(tl.float16))
        tl.store(scales_ptr + global_block_id * 2 + 1, new_log_max.to(tl.float16))
        tl.store(param_ptr + local_offs, p_new, mask=mask)


def triton_multi_tensor_v_update_weight(
    indices: torch.Tensor,
    scales: torch.Tensor,
    grads: torch.Tensor,
    ...
    """Update compressed v and apply Adam's parameter update for a whole group."""
    n_params = param_numels.numel()
    total_blocks = scales.shape[0]
    n_programs = min(total_blocks, 16384)

    _multi_tensor_v_update_weight_kernel[(n_programs,)](
        indices, scales.reshape(-1), grads,
        param_ptrs, m_ptrs, step_seed, bc1, bc2,
        param_block_offsets, param_elem_offsets, param_numels,
        n_params=n_params,
        total_blocks=total_blocks,
        beta2=beta2,
