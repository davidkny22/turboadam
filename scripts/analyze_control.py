...
with open("experiments/results/control_experiment/turboadam_log.jsonl") as f:
    turbo = [json.loads(line) for line in f]
...
for b, t in zip(baseline, turbo):
    ...
    print(
        f"{step:>4} | {b['loss']:>16.4f} | {t['loss']:>15.4f} | {delta:>+5.2f} | {rel:>5.1f}% {marker}"
    )

print()
print(
    f"Baseline  time: {baseline[-1]['elapsed_s']:.1f}s ({baseline[-1]['elapsed_s'] / 500:.2f}s/step)"
)
print(
    f"TurboAdam time: {turbo[-1]['elapsed_s']:.1f}s ({turbo[-1]['elapsed_s'] / 500:.2f}s/step)"
)
print(
    f"Speed delta: {(turbo[-1]['elapsed_s'] / baseline[-1]['elapsed_s'] - 1) * 100:.1f}%"
)
